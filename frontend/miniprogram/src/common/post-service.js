...
const baseUrl = 'api/community/posts/'
...
export default class PostService extends BaseService {
  ...
  async release(post) {
    ...
    post.imgs = JSON.stringify(post.imgs)
  ...
  async analyse(url) {
    ...
    const res = await this.request(baseUrl + 'analyse', { link: url }, 'POST')
    if (res.code === 0) {
      return res.data
    }
    return null
  ...
  async recommendForNew() {
    return await this._list(baseUrl + '/recommend/new', null, 'POST')
  }
  async answers(id, pageIndex, pageSize) {
    return this._list(baseUrl + 'answers', { id, pageIndex, pageSize })
  }
  async details(id) {
    const res = await this.request(baseUrl + `get`, {id: id}, 'POST')
    if (res.code === 0) {
  ...
  async _list(url, options, method = 'POST') {
    const res = await this.request(url, {'params': options}, method)
    if (res.code === 0) {
  ...
  _parsePost(item) {
    ...
    if (item.post && item.post.imgs) {
      item.post.imgs = JSON.parse(item.post.imgs).map(img => {
        const path = img.path
        img.path = this.getImgUrl() + path
        return img
      })
